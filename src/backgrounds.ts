...
import type { BiManaType, ManaType, UnaryManaType } from "./mana.js";
import { isUnaryType, unaryToBiType } from "./mana.js";
  ...
  switch (coloredMana.length) {
    ...
    case 2:
      if (isUnaryType(coloredMana[0]) && isUnaryType(coloredMana[1])) {
        const result = Object.entries(unaryToBiType).find(
          ([_multi, array]) => {
            return (array as readonly string[]).every((type) =>
              (coloredMana as readonly string[]).includes(type),
            );
          },
        );
        if (result) {
...
export function biManaToColor(mana: BiManaType, gold: boolean): CardColor {
  switch (mana) {
    ...
    default:
      throw new Error(`Unknown bi mana type: ${mana}`);
  }
}
...
export function getFrameAndBackgroundFromAspect(aspect: Card["aspect"]): {
  frame: string;
  background: string;
} {
  const { color, frame, legendary } = aspect;
  const fileName = `Frame=${frame}, Color=${color}, Legendary=${legendary ? "Yes" : "No"}, Holo Stamp=Yes.svg`;
  const bg = getBackgroundFromColor(color, frame);

  return {
    frame: fileUrl(path.join(ASSETS.framesDir, fileName)),
    background: fileUrl(path.join(ASSETS.backgroundsDir, `${bg}.png`)),
  };
}
